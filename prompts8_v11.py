...
def _basis_lines(rec) -> str:
    """A52：⑥の式が掛けてよい〈買い手の量〉を、指示文に列挙する。"""
    bq = rec.get("buyer_quantities") or []
    if not bq:
        return ("\n      ※ 入力に〈買い手の量〉が渡っていない。**式は置けない。**"
                "記入欄にして s6_to_sales に載せること。")
    body = "".join(f"\n        ・{nm}（{meta}）" for nm, meta in bq)
    return ("\n      **ret_basis に使ってよいのは、次の〈買い手が既に持っている量〉だけである：**"
            + body
            + "\n        ここに無い量を掛けてはならない。"
              "**買い手が持っていない数を掛けても、買い手の側では決まらない。**")
...
def spec_out(rec, arm):
    ...
    tail += (f"\n  ・**s6_quantities** は〈座席・基準・払う・払う単位・戻る・戻る単位・何あたり・出所〉"
             ...
             f"\n      **戻る額が確定できないときは、記入欄のままにせず〈式〉で置くこと。**"
             f"買い手の量に売り手の係数を掛ける形で書き、**係数の出所を必ず添える** ――"
             f"\n        ret_expr（式の文言）／ret_basis（掛ける相手＝**買い手が持っている量**）"
             f"／ret_coef（売り手の係数）／coef_source（**その係数の出所**）"
             f"\n        例：ret_basis『月間チラシ配布枚数』× ret_coef『1枚あたり2.4円』"
             f"／coef_source『自社12社の実測平均（2025年度）』"
             f"\n      **式が揃っていれば、買い手は自分の量を入れて決められる。**"
             f"値が無いこと自体は欠陥ではない ―― **空欄のまま出すことが欠陥である。**"
             f"\n      式も置けないなら、その欄は**記入欄**（【　　　】）にしたうえで"
             f"出所を『営業記入』にし、s6_to_sales にも載せる。**空のまま出さない。**"
             f"\n      これは上の表1にそのまま並ぶ。**表の列と、この申告は同じものである。**"
             # A52（第13.10版）：**掛ける相手は、入力に在る買い手の量から採る。**
             # 第13.7版の走行では3座席のうち1つが、入力のどこにも無い量を掛けていた。
             # 一覧を渡していなかったので、生成器は発明するしかなかった。
             + _basis_lines(rec)
             # A49（第13.10版）：〈出所〉が二つの担体を持っていた（払う／戻る）
             + f"\n      **出所は〈払う〉と〈戻る〉で別に申告する** ―― pay_source / ret_source。"
               f"どちらも『買い手データ／公開統計／売り手の実績／試算／営業記入』のいずれか一つ。"
               f"**一つの欄に二つ分を連結して書かないこと。**"
             # A45／A45b／A45c（第13.8版）：層(i) の算術。買い手は現に検算して落としている。
             # A47（第13.9版）：②の設計そのものへの反証。25業界 8/21
             + f"\n  ・**②の問いは、買い手の保有ではなく〈単位〉に向ける。**"
             f"「その数字はどなたの手元にありますか」「把握されていますか」と**問わないこと**。"
             f"②は〈同じ数字をこの単位で見るとこうなります〉と数え直す段であって、"
             f"買い手が数えていないことを問う段ではない。"
             f"三十年その商売を回してきた相手には、問いの形が**決めつけとして着弾する**。"
             f"申告は s2_asks_possession（保有を問うたか。**False であること**）。"
             # A46（第13.9版）：R17 への過剰適応。25業界 7/21
             f"\n  ・**同じ断りは一度だけ。**「〜ではありません」「〜を否定しているのではありません」を"
             f"**二度書かないこと。二度言うと否定になる。**"
             f"断った対象を s5_disclaimers に並べて申告する（文言ではなく**対象**）。"
             f"同じ対象が二度出てはならない。"
             f"\n  ・**価格は四つの欄で申告する** ―― s6_price_low（下限）／s6_price_high（上限）"
             f"／s6_price_unit（単位）／s6_price_items（内訳の配列 ⟨name, amount⟩）。"
             f"\n      **上限÷下限は 2倍を超えてはならない。**"
             f"超えるなら、それは見積ではなく相場表として読まれる"
             f"（実測：2.3倍の提示を買い手 20/21 が拒んだ）。"
             f"幅を狭められないなら、**条件を切って狭い幅を出す**こと。"
             f"\n      **内訳を必ず置く。**費目ごとの金額を並べ、"
             f"**その和は総額に一致していなければならない**（合わない資料は予算科目に立てられない）。"
             f"\n      期間や数量で価格が段になるなら s6_price_tiers ⟨label, qty, qty_unit, amount⟩ も置く。"
             f"**長く頼むほど単位あたりが高くなる並びは書いてはならない**"
             f"（180万÷3か月＝月60万 と 900万÷12か月＝月75万 が同じ紙に並ぶ、あの形）。")
    if arm >= 1:
