...
- 4／4 走行と採点 … `wf_v11.js`／`validate8_v11.py`／`regen_v12.py`／`wf_v10.js`／`validate8_v2.py`／`report8_v10.py`／`stamp.py`
...
`sales_logic.py` の sha256[:12] は `c2cdcb1eafd2`（git 2ce14d2 / v12-4）。
回帰テストは `cells8_v2/v6/v6b/v7/v8/v9.py` も import します。それらと走行データ
（`run8_v11.json` ほか）はこの束には入っていません。tar のほうを使ってください。
