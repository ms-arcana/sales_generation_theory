# コード束 2／6 ── 回帰と辞書（第12.8版）

作業一式のコード部分を6分割したものの 2 枚目です。**6枚すべて**を同じフォルダで復元してください。
...
```json
{
 ...
 "judgments": {
  ...
  "A5_KAPPA_PARTIAL": "最終裁定点は複数の基準で見るのに、⑥の量が届いているのは一部だけ。残りの基準でも読める量を置くかどうかを人が決める",
  "N6_CARD_UNKNOWN": "要求の濃度が 1 でも n でもない。四つ組の書き方が壊れている",
  "N6_CARD_UNJUSTIFIED": "その要求を「一つだけ」と置いた理由が書かれていない。未定義の定義域は ⊥ であって「たぶん一つ」ではない",
  "N6_FIELD_SCALAR": "論理式は集合を要求しているのに、申告欄が単数で作られている（型1）。生成器は連結して書く",
  "N6_VALUE_SCALAR": "写像の値が複数ありうるのに、値が単数で作られている（型1 の写像版）",
  "N6_REQ_MISSING": "申告欄はあるのに、それが何についての要求かが書かれていない",
  "N6_SATISFY_MISSING": "何をもって満たしたとするかが書かれていない（「無い」の明示は充足か）",
  "N6_ENTRENCH_TIE": "同じ担体に掛かる要求の強さが同じで、衝突したときの順序が決まらない（型4）"
 },
 "rules": {
  "R1_OK": "D5 に時限あり",
