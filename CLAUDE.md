...
- 仕様 → `SPEC8.md`（第11版まで。A24〜A57・N₄′・N₆追補・R20 が未記載＝債務。**型の側から書けば短くなる**）
...
python3 -c "from sales_logic import audit_retired_reads as c; print(len(c()))"
...
期待値は `345 / 13 / 20 / 13 項目すべて ok` ／ `6 0` ／ `A57 は 0` ／ `8セルとも解が在る` ／
`整合性チェックの (2)〜(5)(7) が「なし」`（**(2) は 209 = 209 の完全一致・第14.3版で返済**）。
...
8. **欄を退役させる前に、読み手を全部 橋へ移す。**`audit_retired_reads()` が 0 になってから
   `REQ_RETIRED` へ入れること。**「吸収済みか」は欄の意味への問いではなく、読み手への問いです**
   （**A57**・第14.4版）。
9. **⊥ かどうかは `is_bottom` でだけ決める。**`UNIT_UNKNOWN` を生で引かないこと
   （記入欄【　　　】が漏れます）。**⊥ を別の欄で代用しない** ―― `a or b` で埋めない。
   担体の一意性は、記号の名前だけでなく**述語**にも要ります（**A55**・第14.3版）。

## この設計で最も多い欠陥

**モデルの誤りではなく、検査の誤りです。**「浅い一致」は **A54** として畳みました（13件）。
