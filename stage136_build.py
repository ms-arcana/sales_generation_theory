# -*- coding: utf-8 -*-
"""第13.6版 ―― 盲検買い手16体の入力を作る。

第13.5版と**同じペルソナ・同じ座席・同じ組み方**を使う。道具を変えない。
違うのは資料の中身だけ（A41・A43・N₄′／R20・形式変更を入れた指示文で生成し直したもの）。
...
V = {r["id"]: r for r in _load("verified136.json")}
DEC = {r["id"]: r for r in _load("decisions8_v12.json")}
PER = _load("persona12.json")
d = pathlib.Path("stage136"); d.mkdir(exist_ok=True)
...
old = {r["id"]: r for r in _load("verified135.json")}
same = [cid for cid in DEC if body_of(cid) == "\n\n".join(
    f"── {s} 枚目 ──\n{old[cid]['copy'].get(s,'')}" for s in DEC[cid]["sigma"])]
print(f"第13.5版と本文が同一のセル：{len(same)}件 {same}")
